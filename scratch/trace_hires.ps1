...
public class HighResLogoTracer {
    public static string ProcessHires(string inPath, string outXmlPath, string outSvgPath, string outPreviewPath) {
        ...
        // Viewport is 108x108.
        // Target icon size ~68dp, centered
        double targetSize = 68.0;
        ...
        // Sort components from top to bottom (by minY)
        components.Sort((a, b) => {
            int minYa = h, minYb = h;
            foreach (var p in a) if (p.Y < minYa) minYa = p.Y;
            foreach (var p in b) if (p.Y < minYb) minYb = p.Y;
        ...
        foreach (var comp in components) {
            List<Point> contour = TraceContour(comp, w, h);
            if (contour.Count < 10) continue;

            // Map to 108x108 space
            List<PointF> mapped = new List<PointF>();
            foreach (var p in contour) {
                float sx = (float)(offsetX + (p.X - minX) * scale);
                float sy = (float)(offsetY + (p.Y - minY) * scale);
                mapped.Add(new PointF(sx, sy));
            }

            // Smooth polygon contour
            List<PointF> smooth = SmoothContour(mapped, 7);

            // Adaptive polygon simplification
            List<PointF> nodes = SimplifyRDP(smooth, 0.55);
            if (nodes.Count < 4) continue;

            // Build smooth Bezier path
            string d = ToCubicBezierPath(nodes);
            allPathBuilder.Append(d).Append(" ");
        ...
        string finalPathData = allPathBuilder.ToString();

        // 1. Android Vector XML
        string xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                     ...
                     "        android:pathData=\"" + finalPathData + "\" />\n" +
                     "</vector>\n";
        ...
        string svg = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                     ...
                     "  <path fill=\"#F5F1E8\" d=\"" + finalPathData + "\"/>\n" +
                     "</svg>";
        System.IO.File.WriteAllText(outSvgPath, svg, Encoding.UTF8);

        // 3. Render Crisp 512x512 Preview
    ...
    private static List<Point> TraceContour(List<Point> comp, int w, int h) {
    ...
    private static List<PointF> SmoothContour(List<PointF> pts, int passes) {
    ...
    private static string ToCubicBezierPath(List<PointF> pts) {
        int n = pts.Count;
...
[HighResLogoTracer]::ProcessHires(
    "c:\Users\abhis\Downloads\MUSIC APP\assets\logo_hires.png",
    "c:\Users\abhis\Downloads\MUSIC APP\scratch\logo_hires_vector.xml",
    "c:\Users\abhis\Downloads\MUSIC APP\assets\logo_hires.svg",
    "c:\Users\abhis\Downloads\MUSIC APP\assets\logo_hires_preview.png"
)
Write-Host "Processed high-res logo into smooth vector!"
